...
void LTAnalyzer::beginEvent(){
  SMPAnalyzerCore::beginEvent();
  if(!IsDATA&&false){
    if(IsDYSample)
      genjets=GetAllGenJets(gens);
    ...
    //PrintGen(gens);
    cout<<"GenLepton"<<endl;
    cout<<gen_l0_bare.Pt()<<"\t"<<gen_l0_bare.Eta()<<"\t"<<gen_l0_bare.Phi()<<"\t"<<gen_l0_bare.E()<<endl;
    cout<<gen_l1_bare.Pt()<<"\t"<<gen_l1_bare.Eta()<<"\t"<<gen_l1_bare.Phi()<<"\t"<<gen_l1_bare.E()<<endl;
    
    cout<<"GenJets"<<endl;
    for(int i=0,n=genjets.size();i<n;i++){
      TLorentzVector genjet(genjets[i].px(),genjets[i].py(),genjets[i].pz(),genjets[i].e());
      bool overlap=false;
      if(genjet.DeltaR(gen_l0_bare)<0.4) overlap=true;
      if(genjet.DeltaR(gen_l1_bare)<0.4) overlap=true;
      //cout<<genjet.Pt()<<"\t"<<genjet.Eta()<<"\t"<<genjet.Phi()<<"\t"<<genjet.E()<<"\t"<<genjets[i].constituents().size()<<endl;
      cout<<(overlap? "*" : "")<<genjet.Pt()<<"\t"<<genjet.Eta()<<"\t"<<genjet.Phi()<<"\t"<<genjet.E()<<endl;
    }
    */
    int gen_njet=0;
    for(int i=0,n=genjets.size();i<n;i++){
      TLorentzVector genjet(genjets[i].px(),genjets[i].py(),genjets[i].pz(),genjets[i].e());
      if(gen_l0_bare.E()&&genjet.DeltaR(gen_l0_bare)<0.4) continue;
      if(gen_l1_bare.E()&&genjet.DeltaR(gen_l1_bare)<0.4) continue;
      if(fabs(genjet.Eta())>5) continue;
	...
	if(jet.DeltaR(gen_l0_bare)<0.4) continue;
	if(jet.DeltaR(gen_l1_bare)<0.4) continue;
	if(jet.Pt()<20) continue;
	gen_njet2++;
      }
    }
    int gen_nparton=0;
    for(int i=0,n=lhes.size();i<n;i++){
      if(lhes[i].Status()!=1) continue;
      if(abs(lhes[i].ID())>6&&lhes[i].ID()!=21) continue;
      if(lhes[i].Pt()<20) continue;
      gen_nparton++;
    }
    int njet=0;
    vector<Jet> jets=GetJets("tightLepVeto",20,5.0);
    for(int i=0,n=jets.size();i<n;i++){
      if(jets[i].DeltaR(gen_l0_bare)<0.4) continue;
      if(jets[i].DeltaR(gen_l1_bare)<0.4) continue;
      njet++;
    }
    //cout<<"gen_njet:"<<gen_njet<<" gen_njet2:"<<gen_njet2<<" gen_nparton:"<<gen_nparton<<" njet:"<<njet<<endl;
