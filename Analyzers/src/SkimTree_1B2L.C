#include "SkimTree_1B2L.h"

void SkimTree_1B2L::initializeAnalyzer(){

  outfile->cd();
  cout << "[SkimTree_1B2L::initializeAnalyzer()] gDirectory = " << gDirectory->GetName() << endl;
  ...
  if(DataYear==2016){
    ...
    cout<<"[SkimTree_1B2L::initializeAnalyzer] DataYear is wrong : " << DataYear << endl;
  }
  ...
  cout << "[SkimTree_1B2L::initializeAnalyzer] triggers to skim = " << endl;
  for(unsigned int i=0; i<double_triggers.size(); i++){
    cout << "[SkimTree_1B2L::initializeAnalyzer]   " << double_triggers.at(i) << endl;
  }
  for(unsigned int i=0; i<single_muon_triggers.size(); i++){
    cout << "[SkimTree_1B2L::initializeAnalyzer]   " << single_muon_triggers.at(i) << endl;
  }
  for(unsigned int i=0; i<single_electron_triggers.size(); i++){
    cout << "[SkimTree_1B2L::initializeAnalyzer]   " << single_electron_triggers.at(i) << endl;
  }

}

void SkimTree_1B2L::executeEvent(){
...
void SkimTree_1B2L::executeEventFromParameter(AnalyzerParameter param){

}

SkimTree_1B2L::SkimTree_1B2L(){
  newtree=NULL;
}
