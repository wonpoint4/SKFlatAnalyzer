...
void plot(){
  ...
  for(TString channel:{"ee","el","mm","mu"}){
    ...
    for(TString era:{"2016a","2016b","2017","2018","Run2"}){
      ...
      TString common=" xmin:-2.5 xmax:2.5 1:ytitle:Events 2:ytitle:'data/Pred.' 2:xtitle:'"+lepton+" #eta' norm 2:ymin:0.92 2:ymax:1.08 rebin:2 ";
      //aa.SavePlotCondor(channel+era_+"/m80to100/leta",common+"sysname:effAN sysdetail 2:sysleg save:"+channel+era+"_leta");
      //aa.SavePlotCondor(channel+era_+"/m80to100/leta",common+"suffix:_noDZSF:sim save:"+channel+era+"_leta_noDZSF");
      if(channel[0]=='m'){
