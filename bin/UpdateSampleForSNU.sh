...
while read line <&3; do
	...
	## get info for MC
	SAMPLE="${array[$(($VI+3))]}"
	ALIAS=$(cat $SKFlat_WD/data/$SKFlatV/$YEAR/Sample/SampleSummary_MC.txt|grep -P "[ \t]${SAMPLE}[ \t]"|awk '{print $1}'|head -n1)
	if [ "$ALIAS" = "" ]; then
	    ALIAS=$(cat $SKFlat_WD/data/$SKFlatV/*/Sample/SampleSummary_MC.txt|grep -P "[ \t]${SAMPLE}[ \t]"|awk '{print $1}'|head -n1)
	fi  
	if [ "$ALIAS" = "" ]; then
	    ALIAS=$(cat $SKFlat_WD/data/$SKFlatV/*/*/SampleSummary_MC.txt|grep -P "[ \t]${SAMPLE}[ \t]"|awk '{print $1}'|head -n1)
	fi  
	if [ "$ALIAS" = "" ]; then
