...
const double RocelecoR_CrystalBall::pi = 3.14159;
const double RocelecoR_CrystalBall::sqrtPiOver2 = sqrt(RocelecoR_CrystalBall::pi/2.0);
const double RocelecoR_CrystalBall::sqrt2 = sqrt(2.0);

RocelecoR_RocRes::RocelecoR_RocRes(){
    reset();
}

void RocelecoR_RocRes::reset(){
    NETA=0;
    NTRK=0;
    NMIN=0;
...
int RocelecoR_RocRes::etaBin(double eta) const{
    double abseta=fabs(eta);
    for(int i=0; i<NETA-1; ++i) if(abseta<resol[i+1].eta) return i;
    return NETA-1;
}

int RocelecoR_RocRes::trkBin(double x, int h, TYPE T) const{
    for(int i=0; i<NTRK-1; ++i) if(x<resol[h].nTrk[T][i+1]) return i;
    return NTRK-1;
}

double RocelecoR_RocRes::Sigma(double pt, int H, int F) const{
    double dpt=pt-45;
    const ResParams &rp = resol[H];
    return rp.rsPar[0][F] + rp.rsPar[1][F]*dpt + rp.rsPar[2][F]*dpt*dpt;
}

double RocelecoR_RocRes::rndm(int H, int F, double w) const{
    const ResParams &rp = resol[H];
    return rp.nTrk[MC][F]+(rp.nTrk[MC][F+1]-rp.nTrk[MC][F])*w; 
}

double RocelecoR_RocRes::kSpread(double gpt, double rpt, double eta, int n, double w) const{
...
double RocelecoR_RocRes::kSpread(double gpt, double rpt, double eta) const{
    int H = etaBin(fabs(eta));
    const auto &k = resol[H].kRes;
    double x = gpt/rpt;
//    cout<< " k[Data]="<<k[Data]<<" k[MC]="<<k[MC]<<" x="<<x<<endl;
    return x + (1.-x)*k[Data]/k[MC]; //x / (1.0 + (x-1.0)*k[Data]/k[MC]);
}

double RocelecoR_RocRes::kSmear(double pt, double eta, TYPE type, double v, double u) const{
    int H = etaBin(fabs(eta));
    int F = trkBin(v, H); 
    const ResParams &rp = resol[H];
...
double RocelecoR_RocRes::kSmear(double pt, double eta, TYPE type, double w, double u, int n) const{
    int H = etaBin(fabs(eta));
    int F = n-NMIN;
...
double RocelecoR_RocRes::kExtra(double pt, double eta, int n, double u, double w) const{
    int H = etaBin(fabs(eta));
    int F = n>NMIN ? n-NMIN : 0;
    const ResParams &rp = resol[H];
...
double RocelecoR_RocRes::kExtra(double pt, double eta, int n, double u) const{
    int H = etaBin(fabs(eta));
    int F = n>NMIN ? n-NMIN : 0;
    const ResParams &rp = resol[H];
...
void RocelecoR::init(std::string filename){
	...
	if(first4=="NSET"){
	    ...
	    ss >> tag >> NPHI; 
	    DPHI=2*RocelecoR_CrystalBall::pi/NPHI;
	}
	else if(first4=="CETA")  {
	    ss >> tag >> NETA;
...
const double RocelecoR::MPHI=-RocelecoR_CrystalBall::pi;
...
double RocelecoR::kGenSmear(double pt, double eta, double v, double u, RocelecoR_RocRes::TYPE TT, int s, int m) const{
